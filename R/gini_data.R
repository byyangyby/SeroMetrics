...
#' \donttest{
#' data(Fonville)
#' gini_data(Fonville, part_col = "Subject Number", val_col = "titer",
#'          group_col = "Sample Year")
#' }
#'
#' @export
gini_data <- function(data, part_col, val_col, group_col = NULL, var_trans = TRUE, base = 2, adj = 10, adjust = NULL, gini_col = "gini_coefficient") {

  if(is.null(group_col)){
    stat <- unique(data[, part_col, drop = FALSE])

    gini_coefficient <- sapply(seq_len(nrow(stat)), function(i) {
      gini_cal(stat[i, part_col], NULL, data, part_col, val_col, group_col, var_trans, base, adj, adjust)
    })
  }else{
    stat <- unique(data[, c(part_col, group_col), drop = FALSE])

    gini_coefficient <- sapply(seq_len(nrow(stat)), function(i) {
      gini_cal(stat[i, part_col], stat[i, group_col], data, part_col, val_col, group_col, var_trans, base, adj, adjust)
    })
  }
...
#' @keywords internal
#' @noRd
gini_cal <- function(i, j, data, part_col, val_col, group_col, var_trans, base, adj, adjust = NULL) {
