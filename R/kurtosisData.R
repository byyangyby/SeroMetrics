...
kurtosisData <- function(data, part_col,weight_col,val_col,group_col = NULL, var_trans = 1, base = 2, adj = 10, mode = NULL, adjust = NULL, kurt_col = "Kurtosis") {
  ...
  if(is.null(group_col)){
    ...
    Kurtosis <- sapply(seq_len(nrow(stat)), function(i) {
      kurtCal(stat[i, part_col], NULL, data, part_col, weight_col,val_col, group_col, var_trans, base, adj, mode, adjust)
    })
  }else{
    stat <- unique(data[, c(part_col, group_col), drop = FALSE])

    Kurtosis <- sapply(seq_len(nrow(stat)), function(i) {
      kurtCal(stat[i, part_col], stat[i, group_col], data, part_col, weight_col,val_col, group_col, var_trans, base, adj, mode, adjust)
    })
  }
  ...
  if(!is.null(mode)){
    if(mode == "mean"){
      ...
      df <- df %>%
        group_by(across(all_of(weight_col))) %>%
        summarise(
          titer = roung(max(.data[[val_col]], na.rm = TRUE)),
          id = first(.data[[part_col]])
        )%>%
        ungroup() %>%
        rename(!!val_col := titer, !!part_col := id)
