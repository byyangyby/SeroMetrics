...
#' \donttest{
#' data(Fonville)
#' prop_data(Fonville, part_col = "Subject Number", val_col = "titer",
#'          group_col = "Sample Year", threshold = 4)
#' }
#'
#' @export
prop_data <- function(data, part_col, val_col, group_col = NULL, var_trans = TRUE, threshold_trans = TRUE, base = 2, adj = 10, threshold = 2, prop_col = "Proportion") {

  if(is.null(group_col)){
    stat <- unique(data[, part_col, drop = FALSE])

    Proportion <- sapply(seq_len(nrow(stat)), function(i) {
      prop_cal(stat[i, part_col], NULL, data, part_col, val_col, group_col = NULL, var_trans, threshold_trans, base, adj, threshold)
    })
  }else{
    stat <- unique(data[, c(part_col, group_col), drop = FALSE])

    Proportion <- sapply(seq_len(nrow(stat)), function(i) {
      prop_cal(stat[i, part_col], stat[i, group_col], data, part_col, val_col, group_col, var_trans, threshold_trans, base, adj, threshold)
    })
  }
...
#' @keywords internal
#' @noRd
prop_cal <- function(i, j, data, part_col, val_col, group_col, var_trans, threshold_trans, base, adj, threshold) {
