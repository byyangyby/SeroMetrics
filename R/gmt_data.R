...
#' \donttest{
#' data(Fonville)
#' gmt_data(Fonville, part_col = "Subject Number", val_col = "titer",
#'         group_col = "Sample Year")
#' }
#'
#' @export
gmt_data <- function(data, part_col, val_col, group_col = NULL, var_trans = TRUE, output_trans = TRUE, base = 2, adj = 10, gmt_col = "GMT") {

  if(is.null(group_col)){
    stat <- unique(data[, part_col, drop = FALSE])

    GMT <- sapply(seq_len(nrow(stat)), function(i) {
      gmt_cal(stat[i, part_col], NULL, data, part_col, val_col, group_col, var_trans, output_trans, base, adj)
    })
  }else{
    stat <- unique(data[, c(part_col, group_col), drop = FALSE])

    GMT <- sapply(seq_len(nrow(stat)), function(i) {
      gmt_cal(stat[i, part_col], stat[i, group_col], data, part_col, val_col, group_col, var_trans, output_trans, base, adj)
    })
  }
...
#' @keywords internal
#' @noRd
gmt_cal <- function(i, j, data, part_col, val_col, group_col, var_trans, output_trans, base, adj) {
