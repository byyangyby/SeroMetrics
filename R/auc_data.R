...
#' \donttest{
#' data(Fonville)
#' auc_data(Fonville, part_col = "Subject Number", weight_col = "isolation_year",
#'         val_col = "titer", group_col = "Sample Year")
#' }
#'
#' @export
auc_data <- function(data, part_col, weight_col, val_col, group_col = NULL, var_trans = TRUE, base = 2, adj = 10, mode = NULL, adjust = NULL, auc_col = "AUC") {

  if(is.null(group_col)){
    stat <- unique(data[, part_col, drop = FALSE])

    AUC <- sapply(seq_len(nrow(stat)), function(i) auc_cal(stat[i, part_col], NULL, data, part_col, weight_col, val_col, group_col, var_trans, base, adj, mode, adjust))
  }else{
    stat <- unique(data[, c(part_col, group_col), drop = FALSE])

    AUC <- sapply(seq_len(nrow(stat)), function(i) {
      auc_cal(stat[i, part_col], stat[i, group_col], data, part_col, weight_col, val_col, group_col, var_trans, base, adj, mode, adjust)
    })
  }
...
#' @keywords internal
#' @noRd
auc_cal <- function(i, j, data, part_col, weight_col, val_col, group_col, var_trans, base, adj, mode, adjust = NULL) {
