...
#' \donttest{
#' data(Fonville)
#' aty_data(Fonville, part_col = "Subject Number", weight_col = "isolation_year",
#'         val_col = "titer", group_col = "Sample Year")
#' }
#'
#' @export
aty_data <- function(data, part_col, weight_col, val_col, group_col = NULL, var_trans = TRUE, base = 2, adj = 10, mode = NULL, adjust = NULL, aty_col = "ATY") {

  if(is.null(group_col)){
    stat <- unique(data[, part_col, drop = FALSE])
    ATY <- sapply(seq_len(nrow(stat)), function(i) aty_cal(stat[i, part_col], NULL, data, part_col, weight_col, val_col, group_col, var_trans, base, adj, mode, adjust))
  }else{
    stat <- unique(data[, c(part_col, group_col), drop = FALSE])
    ATY <- sapply(seq_len(nrow(stat)), function(i) {
      aty_cal(stat[i, part_col], stat[i, group_col], data, part_col, weight_col, val_col, group_col, var_trans, base, adj, mode, adjust)
    })
  }
  stat <- cbind(stat, ATY)
...
#' @keywords internal
#' @noRd
aty_cal <- function(i, j, data, part_col, weight_col, val_col, group_col, var_trans, base, adj, mode, adjust = NULL) {
