...
#' \donttest{
#' data(Fonville)
#' prot_prop_data(Fonville, part_col = "Subject Number", val_col = "titer",
#'               group_col = "Sample Year")
#' }
#'
#' @export
prot_prop_data <- function(data, part_col, val_col, group_col = NULL, min.y = NULL, weight = c(0.00, 0.05, 0.10, 0.30, 0.60, 0.75, 0.85, 0.90, 0.95, 0.99), var_trans = TRUE, weight_trans = TRUE, base = 2, adj = 10, prot_prop_col = "Prot_prop") {

  if(is.null(group_col)){
    stat <- unique(data[, part_col, drop = FALSE])

    Prot_prop <- sapply(seq_len(nrow(stat)), function(i) {
      prot_prop_cal(stat[i, part_col], NULL, data, part_col, val_col, group_col, min.y, weight, var_trans, weight_trans, base, adj)
    })
  }else{
    stat <- unique(data[, c(part_col, group_col), drop = FALSE])

    Prot_prop <- sapply(seq_len(nrow(stat)), function(i) {
      prot_prop_cal(stat[i, part_col], stat[i, group_col], data, part_col, val_col, group_col, min.y, weight, var_trans, weight_trans, base, adj)
    })
  }
...
#' @keywords internal
#' @noRd
prot_prop_cal <- function(i, j,data, part_col, val_col, group_col, min.y, weight, var_trans, weight_trans, base, adj) {
