#' Function to format data and calculate kurtosis
#'
...
#' \donttest{
#' data(Fonville)
#' kurtosis_data(Fonville, part_col = "Subject Number", weight_col = "isolation_year",
#'              val_col = "titer", group_col = "Sample Year")
#' }
#'
#' @export
kurtosis_data <- function(data, part_col, weight_col, val_col, group_col = NULL, var_trans = TRUE, base = 2, adj = 10, mode = NULL, adjust = NULL, kurt_col = "Kurtosis") {

  if(is.null(group_col)){
    stat <- unique(data[, part_col, drop = FALSE])

    Kurtosis <- sapply(seq_len(nrow(stat)), function(i) {
      kurt_cal(stat[i, part_col], NULL, data, part_col, weight_col,val_col, group_col, var_trans, base, adj, mode, adjust)
    })
  }else{
    stat <- unique(data[, c(part_col, group_col), drop = FALSE])

    Kurtosis <- sapply(seq_len(nrow(stat)), function(i) {
      kurt_cal(stat[i, part_col], stat[i, group_col], data, part_col, weight_col,val_col, group_col, var_trans, base, adj, mode, adjust)
    })
  }
...
#' Calculate the kurtosis for a given participant
#'
...
#' @keywords internal
#' @noRd
kurt_cal <- function(i, j, data, part_col, weight_col, val_col, group_col, var_trans, base, adj, mode, adjust) {

  if(is.null(j)){
    if(is.numeric(data[[part_col]])){
      df <- data[data[[part_col]] == as.numeric(i), ]
    } else {
      df <- data[data[[part_col]] == as.character(i), ]
    }
  }else{
    if(is.numeric(data[[part_col]])){
  ...
  if(!is.null(mode)){
    if(mode == "mean"){
      df <- df |>
        ...
        summarise(
          titer = round(mean(.data[[val_col]], na.rm = TRUE)),
      ...
      df <- df |>
        group_by(across(all_of(weight_col))) |>
        summarise(
          titer = round(max(.data[[val_col]], na.rm = TRUE)),
          id = first(.data[[part_col]])
        )|>
        ungroup() |>
        rename(!!val_col := titer, !!part_col := id)
      ...
      df <- df |>
        group_by(across(all_of(weight_col))) |>
        summarise(
          titer = round(min(.data[[val_col]], na.rm = TRUE)),
          id = first(.data[[part_col]])
        )|>
        ungroup() |>
        rename(!!val_col := titer, !!part_col := id)
  ...
  kurt <- kurt(y, x, var_trans, base, adj, adjust)
