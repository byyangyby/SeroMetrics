#' Function to format data and calculate skewness
#'
...
#' \donttest{
#' data(Fonville)
#' skewness_data(Fonville, part_col = "Subject Number", weight_col = "isolation_year",
#'              val_col = "titer", group_col = "Sample Year")
#' }
#'
#' @export
skewness_data <- function(data, part_col, weight_col, val_col, group_col = NULL, var_trans = TRUE, base = 2, adj = 10, mode = NULL, adjust = NULL, skew_col = "Skewness") {

  if(is.null(group_col)){
    stat <- unique(data[, part_col, drop = FALSE])

    Skewness <- sapply(seq_len(nrow(stat)), function(i) {
      skew_cal(stat[i, part_col], NULL, data, part_col, weight_col,val_col, group_col, var_trans, base, adj, mode, adjust)
    })
  }else{
    stat <- unique(data[, c(part_col, group_col), drop = FALSE])

    Skewness <- sapply(seq_len(nrow(stat)), function(i) {
      skew_cal(stat[i, part_col], stat[i, group_col], data, part_col, weight_col,val_col, group_col, var_trans, base, adj, mode, adjust)
    })
  }
...
#' @keywords internal
#' @noRd
skew_cal <- function(i, j, data, part_col, weight_col, val_col, group_col, var_trans, base, adj, mode, adjust) {
  ...
  skew <- skew(y, x, var_trans, base, adj, adjust)
