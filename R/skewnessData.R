...
skewnessData <- function(data, part_col,weight_col,val_col,group_col = NULL, var_trans = 1, base = 2, adj = 10 , mode = NULL, adjust = NULL, skew_col = "Skewness") {
  ...
  if(is.null(group_col)){
    ...
    Skewness <- sapply(seq_len(nrow(stat)), function(i) {
      skewCal(stat[i, part_col], NULL, data, part_col, weight_col,val_col, group_col, var_trans, base, adj, mode, adjust)
    })
  }else{
    stat <- unique(data[, c(part_col, group_col), drop = FALSE])

    Skewness <- sapply(seq_len(nrow(stat)), function(i) {
      skewCal(stat[i, part_col], stat[i, group_col], data, part_col, weight_col,val_col, group_col, var_trans, base, adj, mode, adjust)
    })
  }
