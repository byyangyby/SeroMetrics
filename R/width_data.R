...
#' \donttest{
#' data(Fonville)
#' width_data(Fonville, part_col = "Subject Number", weight_col = "isolation_year",
#'           val_col = "titer", group_col = "Sample Year", threshold = 4)
#' }
#'
#' @export
width_data <- function(data, part_col, weight_col, val_col, group_col = NULL, var_trans = TRUE, threshold_trans = TRUE, base = 2, adj = 10, threshold = 2, mode = NULL, adjust = NULL, width_col = "Width") {

  if(is.null(group_col)){
    stat <- unique(data[, part_col, drop = FALSE])

    Width <- sapply(seq_len(nrow(stat)), function(i) {
      width_cal(stat[i, part_col], NULL, data, part_col, weight_col, val_col, group_col, var_trans, threshold_trans, base, adj, threshold, mode, adjust)
    })
  }else{
    stat <- unique(data[, c(part_col, group_col), drop = FALSE])

    Width <- sapply(seq_len(nrow(stat)), function(i) {
      width_cal(stat[i, part_col], stat[i, group_col], data, part_col, weight_col, val_col, group_col, var_trans, threshold_trans, base, adj, threshold, mode, adjust)
    })
  }
...
#' @keywords internal
#' @noRd
width_cal <- function(i, j, data, part_col, weight_col, val_col, group_col, var_trans, threshold_trans, base, adj, threshold, mode, adjust) {
