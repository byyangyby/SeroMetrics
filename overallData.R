...
#' @param aty_adjust how to adjust titers when calculating ATY, default is NULL
#' @param auc_adjust how to adjust titers when calculating AUC, default is NULL
#' @param gini_adjust how to adjust titers when calculating gini coefficient, default is NULL
#' @param kurt_adjust how to adjust titers when calculating Kurtosis, default is NULL
#' @param skew_adjust how to adjust titers when calculating Skewness, default is NULL
#' @param width_adjust how to adjust titers when calculating Width, default is NULL
...
overallData <- function(data, part_col, weight_col, val_col, group_col = NULL,
                        ...
                        base = 2, adj = 10, mode = NULL, aty_adjust = NULL, auc_adjust = NULL, gini_adjust = NULL,kurt_adjust = NULL, skew_adjust = NULL, width_adjust = NULL,
  ...
  for (i in seq_along(required_metrics)) {
    ...
    if (metric == "ATY") {
      Result <- atyData(data = data,part_col = part_col,weight_col = weight_col, val_col = val_col, group_col = group_col, var_trans = var_trans, base = base, adj = adj, mode = mode, adjust = aty_adjust, aty_col = col_name)
    }else if (metric == "AUC"){
      Result <- aucData(data = data, part_col = part_col,weight_col = weight_col, val_col = val_col, group_col = group_col, var_trans = var_trans, base = base, adj = adj, mode = mode, adjust = auc_adjust, auc_col = col_name)
    }else if (metric == "gini_coefficient"){
      Result <- giniData(data = data, part_col = part_col, val_col = val_col,group_col = group_col, var_trans = var_trans, base = base, adj = adj, adjust = gini_adjust, gini_col = col_name)
    }else if (metric == "GMT"){
      ...
      Result <- kurtosisData(data = data, part_col = part_col,weight_col = weight_col, val_col = val_col,group_col = group_col, var_trans = var_trans, base = base, adj = adj, mode = mode, adjust = kurt_adjust, kurt_col = col_name)
      ...
      Result <- skewnessData(data = data, part_col = part_col,weight_col = weight_col, val_col = val_col, group_col = group_col, var_trans = var_trans, base = base, adj = adj , mode = mode, adjust = skew_adjust, skew_col = col_name)
    }else if (substr(metric, 1, 5) == "Width"){
      threshold <- as.numeric(substr(metric, 6, nchar(metric)))
      Result <- widthData(data = data, part_col = part_col,weight_col = weight_col, val_col = val_col, group_col = group_col, var_trans = var_trans, threshold_trans = 1, base = base, adj = adj, threshold = threshold, mode = mode, adjust = width_adjust, width_col = col_name)
    }
